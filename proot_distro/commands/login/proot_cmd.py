...
def build_proot_args(
    *,
    ...
    # Some proot builds (5.1.0 via the Nix wrapper, seen on Replit-style
    # sandboxes) segfault parsing the attached "--cwd=value" long-option
    # syntax. The short option with the value as a separate argv token
    # ("-w", value) hits a different code path and does not crash.
    args += ["-w", login_wd]
